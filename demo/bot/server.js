...
const WEBAPP_URL = process.env.WEBAPP_URL; // optional — enables the "Open Mini App" button
const STICKER_FILE_ID = process.env.STICKER_FILE_ID; // optional — only enables /sticker if set
...
function buildStartKeyboard() {
  const row1 = [];
  if (WEBAPP_URL) {
    row1.push({ text: 'Open Mini App', web_app: { url: WEBAPP_URL } });
  }
  row1.push({ text: 'Docs', url: 'https://github.com/mr-m-apps/tg-kit' });
  return {
    inline_keyboard: [
      row1,
...
async function handleHelp(message) {
  ...
  if (STICKER_FILE_ID) lines.push('/sticker - send a sample sticker');
  await bot.sendMessage({ chat_id: message.chat.id, text: lines.join('\n') });
}
...
async function handleSticker(message) {
  if (!STICKER_FILE_ID) {
    await bot.sendMessage({
      chat_id: message.chat.id,
      text: 'Set STICKER_FILE_ID env var to enable /sticker.',
    });
    return;
  }
  await bot.sendSticker({ chat_id: message.chat.id, sticker: STICKER_FILE_ID });
}
...
async function handleMessage(message) {
  ...
  if (!text) return;
  const command = text.split(' ')[0].split('@')[0]; // strip "@BotName" suffix in groups
...
app.post(WEBHOOK_PATH, async (req, res) => {
  ...
  // Acknowledge immediately, then process — Telegram retries on non-2xx.
  res.status(200).end();
  ...
  try {
    ...
    if (error instanceof TelegramApiError) {
      console.error('Telegram API error', error.errorCode, error.message, error.parameters);
    } else {
...
async function bootstrap() {
  ...
  if (STICKER_FILE_ID) {
    commandList.push({ command: 'sticker', description: 'Send a sample sticker' });
  }

  await bot.setMyCommands({ commands: commandList });
